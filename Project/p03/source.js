...
// Projection and initial view configurations
...
// Reflection transform for mirrored rendering
...
function render(timeStamp) {
    ...
    // Compute light and orbiting camera positions
    ...
    // Select current view based on user selection
    var V;
    if (viewPoint == 0) {
        V = defaultV;
    } else if (viewPoint == 1) {
    ...
    // Calculate teapot's model matrix, including bouncing
    ...
    // Use stencil to render reflection correctly
    ...
    // Set stencil to only draw where ground was drawn
    ...
    // Draw the original teapot (not mirrored)
    gl.useProgram(teapotProgram);
    initAttributeVariable(teapotProgram.vertexBuffer);
    initAttributeVariable(teapotProgram.colorBuffer);
    initAttributeVariable(teapotProgram.normalBuffer);
...
function animate(timeStamp) {
    if (!g_drawingInfo) {
        // OBJ and MTLs are available, initialize buffers
...
window.onload = async function init() {
    ...
    // UI Controls for bouncing, orbiting, and viewpoint
