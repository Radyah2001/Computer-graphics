...
// Teapot default transformations
...
function render(timeStamp) {
    ...
    // Calculate light and orbit-based view matrices
    ...
    let orbitFactor = 1.5;
    let orbitV = lookAt(
        vec3(2*Math.cos(orbit_theta * orbitFactor), 1.0, -3.0 + 2*Math.sin(orbit_theta * orbitFactor)),
        vec3(0.0, -1.0, -3.0),
        vec3(0.0, 1.0, 0.0)
    );

    // Select current view based on viewpoint selection
    let V;
    if (viewPoint == 0) V = defaultV;
    else if (viewPoint == 1) V = orbitV;
    else if (viewPoint == 2) V = lightV;

    // Calculate teapot model matrix with bouncing
