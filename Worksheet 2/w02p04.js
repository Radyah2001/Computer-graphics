...
window.onload = function init() {    
    ...
    let circleCenter = null;   // Track the center point for circles
    ...
    document.getElementById("circle-mode-btn").addEventListener("click", function() {
        drawingMode = 'circle';
        circleCenter = null; // Clear any previous circle center
    });
    ...
    canvas.addEventListener("click", function(event) {
        ...
        if (drawingMode === 'point') {
            ...
            if (circleCenter === null) {
                ...
                circleCenter = vec3(x, y, 0.0);
            } else {
                // Second click: Calculate the circle's radius and add the circle
                let radius = Math.sqrt(Math.pow(x - circleCenter[0], 2) + Math.pow(y - circleCenter[1], 2));
                addCircle(circleCenter, radius, pointColor);
                circleCenter = null; // Clear the center for the next circle
            }
        }
    ...
    function addCircle(center, radius, color) {
        const numSegments = 36; // Number of segments to approximate the circle
        ...
        for (let i = 0; i < numSegments; i++) {
            ...
            // Add vertices for the triangle (center, vertex1, vertex2)
            pointsArray.push(center);
            pointsArray.push(vec3(x1, y1, 0.0));
            pointsArray.push(vec3(x2, y2, 0.0));
    
            // Add color for each vertex
            colorsArray.push(vec4(color));
            colorsArray.push(vec4(color));
            colorsArray.push(vec4(color));
        }
    }
