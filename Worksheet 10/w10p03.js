...
// Current camera parameters that will be updated
var eye = vec3(0, 0, -5);
var at  = vec3(0, 0, 0);
...
function updateViewMatrix(gl, program) {
    if (rotMode) {
        ...
        // Dolly in and out by modifying eye0[2]
        if (eye0[2] + diff >= 0) {
            eye0[2] = 0;
        } else {
            eye0[2] += diff;
...
function initEventHandlersForCanvas(canvas, qrot, qinc) {
    var dragging = false;         
    var lastX = -1, lastY = -1;  
...
window.onload = function init() {
    ...
    initEventListeners();
    initEventHandlersForCanvas(canvas, q_rot, q_inc);
